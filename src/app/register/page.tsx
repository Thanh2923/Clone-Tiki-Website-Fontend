    ...
    const onSubmit = async (data: { email: string, password: string }) => {
      ...
      try {
        ...
        if (error instanceof Error) {
          setMessage("error"); // Lấy thông điệp lỗi
        } else {
          setMessage("error");
        }
      }
      ...
      <div className="w-[90%] mt-5 bg-white shadow-lg rounded-lg overflow-hidden lg:flex">
        ...
        <div className="w-full  lg:w-[45%] p-8">
          ...
          <div className="flex justify-between items-center mt-6">
            <span className="text-sm text-gray-500">Đã có tài khoản? <Link href="/login" className="text-blue-600 font-semibold">Đăng nhập ngay</Link></span>
          </div>
        </div>
      </div>
    </div>
